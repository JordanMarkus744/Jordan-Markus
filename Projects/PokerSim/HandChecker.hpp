...
public:
    bool cardChecker(std::vector<std::string> hand)
    {
        for (auto itr : hand)
        {
            if (!(itr.size() == 2 && // this if statement checks to see if everything is formatted right.
    ...
    void GenerateGames(std::vector<std::string> hand, std::vector<std::string> pile = {})
    {   // The pile vector is new and may go away, but it's so the user can input any cards that
        ...
        { // This is the for loop that will generate every simulated poker match. Each poker match will have
          // 5 other 'Players' and a randomly generated pile. Then it will determine if the user's hand wins or loses
            ...
            std::cout << "\n\nCurrent Pile Size = " << pile.size() << std::endl;

            for (int i = 0; i < currentPile.size(); i++)
            { // this makes sure that any card that is present in the pile is accounted for in the currentDeck
                for (int j = 0; j < CurrentDeck.size(); j++)
                {
            ...
            for (int i = currentPile.size(); i < 5; i++)
            { // if currentPile.size() == 3, then we want to generate 2 more cards to put into the pile.
                int index = rand() % 56;
            ...
            { // Generating the Players
                int c1 = rand() % 56;
                while (CurrentDeck[c1] == "")
                { // if the card is already taken, then loop until we find a card that hasn't been taken
                    c1 = rand() % 56;
                }
